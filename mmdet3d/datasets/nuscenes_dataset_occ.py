...
class NuScenesDatasetOccpancy(NuScenesDataset):
    ...
    def evaluate(self, occ_results, runner=None, show_dir=None, save_dir=None, save_depth_dir=None, **eval_kwargs):
        print('\nStarting Evaluation...')
        ...
        ## Save Depth ##
        if save_depth_dir is not None:
            self.save_render_results(occ_results, save_depth_dir)
    ...
    def save_render_results(self, results, out_path):
        ...
        for index, output in enumerate(results):
            ...
            depth = output['rendered_depths']

            all_depths[token] = depth

        for token, preds in all_depths.items():
            out_file_depth = osp.join(out_path, f'{token}.npz')
            np.savez(out_file_depth, preds)
