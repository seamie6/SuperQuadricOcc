...
class GaussianRectifier(nn.Module):
    def __init__(self, in_channels, rectified_params = ['mean', 'rotation', 'scale', 'eps1', 'eps2' 'opacity'], scale_range=[0.01, 3.2], eps_range=[0.1, 2.0], scale_range_divider=3, eps_range_divider=3,
        ...
        self.scale_min, self.scale_max = [0.01, 3.2]
        self.eps_min, self.eps_max = [0.1, 2.0]
        ...
        if residual_properties:
            ...
            if eps_range is not None:
                self.eps_range = [-(eps_range[1] - eps_range[0])/eps_range_divider, (eps_range[1] - eps_range[0])/eps_range_divider]
            else:
                self.eps_range = None
        else:
        ...
        self.use_eps1 = 'eps1' in rectified_params
        if self.use_eps1:
            eps1_modules = [
                nn.Linear(in_channels, in_channels),
                nn.LayerNorm(in_channels),
            ...
            if self.eps_range is not None:
                eps1_modules.append(nn.Sigmoid())
            self.eps1_head = nn.Sequential(*eps1_modules)
        else:
            self.eps1_head = None

        self.use_eps2 = 'eps2' in rectified_params
        if self.use_eps2:
            eps2_modules = [
                nn.Linear(in_channels, in_channels),
                nn.LayerNorm(in_channels),
                ...
                nn.Linear(in_channels, 1)]
            if self.eps_range is not None:
                eps2_modules.append(nn.Sigmoid())
            self.eps2_head = nn.Sequential(*eps2_modules)
        else:
            self.eps2_head = None
    ...
    def forward(self, means, quats, scale, eps1, eps2, opacity, feature):
        # Update Means
        if self.use_mean:
        ...
        if self.use_eps1:
            eps1_residual = self.eps1_head(feature)
            if self.eps_range is not None:
                eps1_residual = self.eps_range[0] + (self.eps_range[1] - self.eps_range[0]) * eps1_residual
            if self.residual_properties:
                eps1 = torch.clamp(eps1 + eps1_residual, self.eps_min, self.eps_max)
            elif self.combined_properties:
                eps1 = torch.clamp(.5*eps1 + .5*eps1_residual, self.eps_min, self.eps_max)
            else:
                eps1 = eps1_residual

        if self.use_eps2:
            eps2_residual = self.eps2_head(feature)
            if self.eps_range is not None:
                eps2_residual = self.eps_range[0] + (self.eps_range[1] - self.eps_range[0]) * eps2_residual
            if self.residual_properties:
                eps2 = torch.clamp(eps2 + eps2_residual, self.eps_min, self.eps_max)
            elif self.combined_properties:
                eps2 = torch.clamp(.5*eps2 + .5*eps2_residual, self.eps_min, self.eps_max)
            else:
                eps2 = eps2_residual
        ...
        return means, quats, scale, eps1, eps2, opacity
